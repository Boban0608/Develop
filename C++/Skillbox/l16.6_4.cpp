...
Где notes — это целое число, один звук нашей мелодии. Она состоит из 12 звуков и имеет вид int 
melody[12];
*/
...
vector<int> melody;
void RecMelody(string input){
    int accord = 0;
    if (input.length() != 3){
        cout << "Wrong input! 123 for example.\n";
    } else {
        for (int i = 0; i < 3; i++){
            if (input[i] == '1') accord |= DO;
            else if (input[i] == '2') accord |= RE;            
            else if (input[i] == '3') accord |= MI;            
            else if (input[i] == '4') accord |= FA;            
            else if (input[i] == '5') accord |= SOL;
            else if (input[i] == '6') accord |= LA;
            else if (input[i] == '7') accord |= SI;
            else {
                cout << "Wrong notes input! 1-7 possible!"; 
                break;
            }
        }
    }
...
void PlayMelody(){
    for (int i = 0; i < 4; i++){
        for (int j = 0; j < 1; j++){
            if (melody[i] & DO) cout << "DO ";
            if (melody[i] & RE) cout << "RE ";
            if (melody[i] & MI) cout << "MI ";
            if (melody[i] & FA) cout << "FA ";
            if (melody[i] & SOL) cout << "SOL ";
            if (melody[i] & LA) cout << "LA ";
            if (melody[i] & SI) cout << "SI ";
        }
        cout << endl;
    }
}
main() {
  cout << "----------------\n";
  cout << "Mechanical piano\n";
  cout << "----------------\n";
  string notes_set;
  int count = 0;
  do {
    cout << "Input notes set: ";
    cin >> notes_set;
    RecMelody(notes_set);
  } while(melody.size() < 4);
  PlayMelody();
