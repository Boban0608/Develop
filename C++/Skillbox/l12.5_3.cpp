...
и отрицательную скорость. Вызовите эту функцию в main. Убедитесь, что падение происходит за 
счёт assert. Не забудьте подключить заголовочный файл cassert.
Что оценивается
...
#include <cassert>
using namespace std;
float travelTime(float dist, float speed){
    assert(speed > 0);
...
int main() {
  ...
  cout << "Enter speed: \n";
  cin >> speed;
  cout << "Travel time is " << travelTime(distance, speed);
  return 0; 
}